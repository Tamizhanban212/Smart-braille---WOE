...
void testingMode() {
  int randomIndex = random(0, 26);  // Generate a random index from 0 to 25
  const int (*pattern)[2] = letters[randomIndex];  // Get the pattern for the randomly selected alphabet
  displayLetter(pattern);
  delay(dT);
}
